...
function onClickHandler(ev, div, image) {
	var el = window._protected_reference = document.createElement("input");
	el.type = "file";
	el.accept = "image/*,video/*,.pdf,.csv";
	...
	el.addEventListener('change', function (ev2) {
		...
		if (el.files.length) {
			...
			if (file) {
				...
				var url = parseUrl(el.getData('Text'));
				ev.stopPropagation();
				image.src = url.href;
...
// // Download xxSankakucomplex post page
...
function handleFile(div, image) {
	...
	switch (image.file.type) {
		...
		case 'application/pdf':
			//Todo
			image.dom = document.createElement('embed');
			image.dom.setAttribute('with', '100%');
			image.dom.setAttribute('hight', '100%');
			image.dom.setAttribute('style', 'width: 100%;height: 100%');
			image.dom.muted = true;
			// getImageData(image.file, function(rs) {
			// 	console.log(rs)
			// });
			console.log("======pdf=======");
			console.log(image)
	...
	// $.get('/search/', { file: image.file }, function (data) {
	// 	console.log(data);
	// });
			...
			if (image.file.type.startsWith('image')) {
				...
				if (image.file.type == 'application/pdf') {
					...
					fileReaderBuffer.onload = function (rs) {
						image.dataUrl = rs.target.result;
						comparePdfs();
					};
					fileReaderBuffer.readAsArrayBuffer(image.file);
...
async function comparePdfs() {
	...
	if ((image1.dataUrl !== null) && (image2.dataUrl !== null)) {
		...
		// };
		// xhr.send(formData);
		// xhr.send(`pdfActual=${image1.dataUrl}&pdfBaseline=${image2.dataUrl}`);
		// return displayRight(true);
		var upload = async () =>{
			let formData = new FormData();
			formData.append('pdfActualfiles', image1.file);
			formData.append("pdfBaseline", image2.file);
		...
		await upload();
		// await upload('pdfBaseline', image1);
